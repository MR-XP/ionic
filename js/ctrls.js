...
app.controller('AppCtrl', ['$scope','$window','$sce','$rootScope','ajaxService','$location',
    			  function($scope,$window,$sce,$rootScope,ajaxService,$location) {
 ...
 */   
    ...
    $rootScope.clickDateMenu = function (divName,stripName,day,v,cb,handle){
				...
				//返回<ion-content>头部
				handle && $ionicScrollDelegate.$getByHandle(''+handle+'').scrollTop(false);
				
				//回调
				cb && cb(day);
    }
		//导航切换
		$rootScope.footNavMenu = function(v){
			$scope.app.footNav = v;
		}
	  //you know
    ...
    $rootScope.scanQRCode = function(){
		...
		//跳转至高德地图
		$rootScope.mapUrl=function(lng,lat,name){
			window.location.href ="https://uri.amap.com/marker?position="+lng+","+lat+"&name="+name+"";
		}
		//只允许输入手机号
		$rootScope.onlyNum=function(num){
			num=num.replace(/\D/gi,'');
			return num;
		}
		//去除小数点后几位
		$rootScope.removeD=function(num,leg){
			num = parseFloat(num);
			if(!leg)
				leg = 0;
			return num.toFixed(leg);
		}
		
}]);

app.controller("showCtrl", ["$scope","$timeout","$interval","$state","$stateParams","$rootScope","$location","$filter","$compile","$ionicLoading","$ionicPopup","$ionicModal","$ionicScrollDelegate","ajaxService",
				   function ($scope,
							$timeout,
							$interval, 
							$state,
...
app.controller("regCtrl", ["$scope", "$ionicLoading", "ajaxService", "$timeout", "$ionicPopup","$location","$popup","$local",
				  function($scope, $ionicLoading, ajaxService, $timeout, $ionicPopup,$location,$popup,$local) {
		...
		$scope.settime = function() {
			...
			if(countdown <= 0) {
				...
				$('.ipt-phone').prop("disabled", false).css('opacity','1');
				$('.ipt-code').val('获取验证码');
				countdown = 60;
				return;
			} else {
				...
				$('.ipt-phone').prop("disabled", true).css('opacity','0.4');
		...
		//替换多余非数字
		$scope.relCn=function(txt,type){
			if(type == 'phone')
				$scope.user.phone=txt.replace(/\D/gi,'');			
			if(type == 'code')
...
app.controller("guideCtrl", ["$scope", "$ionicLoading", "$timeout", "$location", "$local","ajaxService","$popup","$ionicScrollDelegate",
					function($scope, $ionicLoading, $timeout, $location, $local, ajaxService,$popup,$ionicScrollDelegate) {

		//更新生日显示
		$scope.showDate=function(){
			$scope.dateShow = $('input.ipt').val();
		}
		
		//保存操作记录
		$scope.keep = function(){
			$scope.data.part=[];
			for(var i in $scope.part){
				if($scope.part[i].type){
					$scope.data.part.push($scope.part[i].name);
				}
			}
			$local.setObj('userData',$scope.data);
		}
		
		//获取值
		var px;
		$scope.getPX=function(name,unit){
			px = $ionicScrollDelegate.$getByHandle(''+name+'').getScrollPosition().left;
			$('#'+name+'').html(Math.ceil(px / 10)+unit);
			$scope.data[name] = Math.ceil(px / 10);
		}
		...
		//缓存加载
		if($local.getObj('userData')){
			$scope.data = $local.getObj('userData');
			$scope.data.birthday = new Date($scope.data.birthday);
			$scope.dateShow = $scope.data.birthday.getFullYear()+'-'+($scope.data.birthday.getMonth()+1)+'-'+$scope.data.birthday.getDate();
			$('#height').html($scope.data.height+'cm');
			$('#weight').html($scope.data.weight+'kg');
			$('#target_weight').html($scope.data.target_weight+'kg');
			
			for(var i in $scope.part){
				if($.inArray($scope.part[i].name,$scope.data.part) != -1){
						$scope.part[i].type = true;
				}
			}
/*		此代码失效，不影响使用，后期再检查原因
 * 		$ionicScrollDelegate.$getByHandle('height').scrollTo(($scope.data.height * 10),0,true);
			$ionicScrollDelegate.$getByHandle('weight').scrollTo(($scope.data.weight * 10),0,true);
			$ionicScrollDelegate.$getByHandle('target_weight').scrollTo(($scope.data.target_weight * 10),0,true);*/
		...
		var AcgLg = [];
		//加载动画
		function loadAcg(name,ix,clsName,time){
				AcgLg[ix] = setTimeout(function(){
					angular.element('.'+name+':eq('+ix+')').addClass(''+clsName+'');
				},time)
		}
		...
		$scope.$on('$ionicView.afterEnter', function() {
			$ionicLoading.hide();
			var Acg = angular.element('.'+$scope.pageName+'').length;
			for(var i =0;i<Acg;i++){
				loadAcg($scope.pageName,i,angular.element('.'+$scope.pageName+':eq('+i+')').attr('clsname'),angular.element('.'+$scope.pageName+':eq('+i+')').attr('time'))
			}
		});
